#include "graph.h"
#define Capacity 2
int GetValue(int l, int c, Graph board)
{
...
void sortVector(Vector *vector)
{
    for(int i = 0; i < vector->count - 1 ; i++)
        for(int j = i + 1; j < vector->count; j++)
            if(vector->value[i].l > vector->value[j].l || (vector->value[i].l == vector->value[j].l && vector->value[i].c > vector->value[j].c))
            {
                Coord aux = vector->value[i];
                vector->value[i] = vector->value[j];
                vector->value[j] = aux;
            }
}
...
void LongestHamiltonian(FILE* output_file, Graph board)
{
    ...
    for(int j = 0; j < Islands.count; j++){
        ...
        if (Chain.size <= island->count) {
            ...
            Chain.value = (Coord*) realloc(Chain.value, (Chain.size + 1) * sizeof(Coord));
        }
